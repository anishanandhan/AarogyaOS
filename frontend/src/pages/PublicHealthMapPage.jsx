...
import { MapPin, Activity, Heart, Users, CheckCircle, TrendingUp, Stethoscope, Layers, AlertTriangle, Satellite, Droplets, Sprout, Wind, Loader2 } from 'lucide-react';
    ...
    const initMap = () => {
      ...
      if (viewMode === 'markers') {
        ...
        centres.forEach(centre => {
          const loc = centreLocations[centre.id] || { lat: 12.94, lng: 79.13 };
          bounds.extend(loc);

          const color = centre.healthScore >= 70 ? "#10B981" : // emerald
                        centre.healthScore >= 40 ? "#F59E0B" : // warning
                        "#EF4444"; // danger
          ...
          marker.addListener("click", () => {
            const bedPercent = Math.round((centre.bedsOccupied / centre.bedsTotal) * 100);
            infoWindow.setContent(`
              <div style="color: #0F172A; font-family: system-ui, sans-serif; padding: 6px; min-width: 150px;">
                <h4 style="margin: 0 0 2px 0; font-size: 13px; font-weight: 700;">${centre.name}</h4>
                <p style="margin: 0 0 6px 0; font-size: 10px; font-weight: 600; text-transform: uppercase; color: #64748B;">
                  ${centre.type} · ${centre.block}
                </p>
                <div style="font-size: 11px; line-height: 1.5; color: #334155;">
                  <strong>Health Score:</strong> <span style="color: ${
                    centre.healthScore >= 70 ? '#10B981' : centre.healthScore >= 40 ? '#D97706' : '#EF4444'
                  }; font-weight: 700;">${centre.healthScore}/100</span><br/>
                  <strong>Bed Occupancy:</strong> ${centre.bedsOccupied}/${centre.bedsTotal} (${bedPercent}%)<br/>
                  <strong>Doctors:</strong> ${centre.doctorsPresent}/${centre.doctorsOnRoll} present
                </div>
              </div>
            `);
        ...
        // Plot Heatmap showing critical density zones (low health scores)
        const heatmapData = centres.map(centre => {
          const loc = centreLocations[centre.id] || { lat: 12.94, lng: 79.13 };
          bounds.extend(loc);
          return {
            location: new window.google.maps.LatLng(loc.lat, loc.lng),
            weight: (100 - centre.healthScore) / 10 // heavier weight for critical/worse scores
          };
        });
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="flex flex-col gap-6">
          ...
          <div className="rounded-2xl border border-border-col bg-surface p-5 flex-1">
            <h3 className="text-sm font-bold text-text-secondary uppercase tracking-wider mb-4">Facility Audits</h3>
            ...
            {selectedCentre ? (
              <div className="space-y-4">
                <div>
                  ...
                  <span className="inline-flex items-center rounded-md bg-emerald/10 border border-emerald/20 px-2 py-0.5 text-xs font-semibold text-emerald uppercase mt-1">
                    {selectedCentre.type} · {selectedCentre.block}
                  </span>
                </div>
              ...
              <div className="h-full flex items-center justify-center text-center p-4">
                <div>
                  ...
                  <p className="text-xs text-text-secondary">Click any pin or marker on the map to inspect live facility telemetry.</p>
                </div>
              </div>
            )}
          </div>
